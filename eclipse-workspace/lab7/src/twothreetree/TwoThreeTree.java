...
public class TwoThreeTree<K extends Comparable<K>, V> {
	...
	private Node<K,V> insert(Node<K,V> node, K key, V value) {
		...
		} else if (node.rkey() == null ||
				lessThan(key, node.rkey())) {
	...
	public Node<K,V> add(Node<K,V> currNode, Node<K,V> newNode) {
		...
		if (currNode.rkey() == null) {
			...
			// add to right
			if (largerThan(key, currNode.lkey()) ||
				equalTo(key, currNode.lkey())) {
		...
		// add middle
			...
			Node<K,V> currNodeRightSubtree = currNode.right();
